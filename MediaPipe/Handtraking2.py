...
def main():
    ...
    while True:
        ...
        if len(lmList) != 0:
            A1 = (lmList[4][1], lmList[4][2])
            A2 = (lmList[3][1], lmList[3][2])
            A3 = (lmList[2][1], lmList[2][2])

            Ag = calcAngLine(A1, A2, A3)
            if 0.5 < Ag < 5:
                e = 1
                AC = (255, 255, 255)
            else:
            ...
            B1 = (lmList[8][1], lmList[8][2])
            B2 = (lmList[7][1], lmList[7][2])
            B3 = (lmList[6][1], lmList[6][2])

            cv2.line(img, A1, A2, AC, 4)
            cv2.line(img, A2, A3, AC, 4)

            Bg = calcAngLine(B1, B2, B3)
            if 0.5 < Bg < 5:
                BC = (255, 255, 255)
                if e == 1:
                    d = distPoints(A1, B1)
            ...
            cv2.line(img, B1, B2, BC, 4)
            cv2.line(img, B2, B3, BC, 4)

            cv2.putText(img, str(d),
                        (int((A1[0] + B1[0]) / 2), int((A1[1] + B1[1]) / 2)),
